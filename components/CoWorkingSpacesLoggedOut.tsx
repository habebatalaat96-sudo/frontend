...
export const CoWorkingSpacesLoggedOut: React.FC<CoWorkingSpacesLoggedOutProps> = ({ onNavigate }) => {

  const [sponsoredSpaces, setSponsoredSpaces] = React.useState<any[]>([]);
  const [nonSponsoredSpaces, setNonSponsoredSpaces] = React.useState<any[]>([]);
  ...
  React.useEffect(() => {
    const fetchSpaces = async () => {
      try {
        ...
        setSponsoredSpaces(all.filter((s: any) => s.sponsored === true));
        setNonSponsoredSpaces(all.filter((s: any) => s.sponsored !== true).slice(0, 6));
      } catch (err) {
        console.log(err);
      } finally {
        setLoading(false);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600">
            Discover the best co-working spaces near you
          </p>
        </div>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-2 mb-6">
            ...
            <p className="text-gray-500 text-sm">Premium co-working space listings</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sponsoredSpaces.map((space) => (
              <Card key={space.id} className="overflow-hidden bg-white/80 backdrop-blur-sm border-white/20 hover:shadow-xl transition-all duration-300 group cursor-pointer">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">{space.rating}</span>
                  </div>
                </div>
              </Card>
            ))}
        ...
        <div className="mb-8">
          ...
          <div className="relative">
            <div className="blur-sm pointer-events-none select-none">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {nonSponsoredSpaces.map((space) => (
                  <Card key={space.id || space._id} className="overflow-hidden bg-white/80 backdrop-blur-sm border-white/20">
                    <div className="relative h-48 overflow-hidden">
                      <img
                        src={space.image}
                        alt={space.name}
                    ...
                    <div className="p-5">
                      <h3 className="text-xl mb-3 text-gray-800">{space.name}</h3>
                      <div className="flex items-start gap-2 mb-3 text-gray-600">
                        <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0 text-cyan-600" />
                        <span className="text-sm">{space.location}</span>
                      </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-white/40 backdrop-blur-[2px]">
              <div className="text-center bg-white/95 backdrop-blur-md p-8 rounded-3xl shadow-2xl max-w-md mx-4 border border-gray-200">
                ...
                <h3 className="text-2xl mb-3 bg-gradient-to-r from-cyan-600 to-purple-600 bg-clip-text text-transparent">
                  Login to View All Co-working Spaces
                </h3>
                <p className="text-gray-600 mb-6">
                  Access our complete directory of co-working spaces with AI-powered recommendations tailored to your needs
                </p>
                <Button
