...
export const RestaurantsLoggedOut: React.FC<RestaurantsLoggedOutProps> = ({ onNavigate }) => {
const [sponsoredRestaurants, setSponsoredRestaurants] = React.useState<any[]>([]);
const [nonSponsoredRestaurants, setNonSponsoredRestaurants] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchRestaurants = async () => {
    try {
      ...
      setSponsoredRestaurants(all.filter((r: any) => r.sponsored === true));
      setNonSponsoredRestaurants(all.filter((r: any) => r.sponsored !== true).slice(0, 6));
    } catch (err) {
      console.log(err);
    } finally {
      setLoading(false);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-2 mb-6">
            ...
            <p className="text-gray-500 text-sm">Premium restaurant listings</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sponsoredRestaurants.map((restaurant) => (
              <Card key={restaurant.id} className="overflow-hidden bg-white/80 backdrop-blur-sm border-white/20 hover:shadow-xl transition-all duration-300 group cursor-pointer">
        ...
        <div className="mb-8">
          ...
          <div className="relative">
            <div className="blur-sm pointer-events-none select-none">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {nonSponsoredRestaurants.map((restaurant) => (
                  <Card key={restaurant.id} className="overflow-hidden bg-white/80 backdrop-blur-sm border-white/20">
                    <div className="relative h-48 overflow-hidden">
                      <img
                        src={restaurant.image}
                        alt={restaurant.name}
                    ...
                    <div className="p-5">
                      <h3 className="text-xl mb-3 text-gray-800">{restaurant.name}</h3>
                      <div className="flex items-start gap-2 mb-3 text-gray-600">
                        <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0 text-cyan-600" />
                        <span className="text-sm">{restaurant.location}</span>
                      </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-white/40 backdrop-blur-[2px]">
              <div className="text-center bg-white/95 backdrop-blur-md p-8 rounded-3xl shadow-2xl max-w-md mx-4 border border-gray-200">
                ...
                <h3 className="text-2xl mb-3 bg-gradient-to-r from-cyan-600 to-purple-600 bg-clip-text text-transparent">
                  Login to View All Restaurants
                </h3>
                <p className="text-gray-600 mb-6">
                  Access our complete directory of restaurants and cafes with AI-powered recommendations tailored to your needs
                </p>
                <Button
